...
\end{frame}
 ...
 \begin{frame}{Human vs. Computer}
   \[
    \alpha A=c
   \]
  \begin{block}{xSCAL() Sacle A Vector }
   \begin{itemize}
    \item<2-> 2n Flops
    \item<3-> 3n Refernces
    \item<4-> q=2/3
...
\note{
\begin{itemize}
 \item \texttt{pgcc -DACML -DBLAS1 -DBLAS2 blasSpeeds.c -lacml -pgf90libs -lpgftnrtl}
...
\note{
\begin{itemize}
 \item \texttt{pgcc -DACML -DBLAS3 blasSpeeds.c -lacml -pgf90libs -lpgftnrtl}
 \item \texttt{pgi/7.2 acml/4.1.0}
 \item opt2218  2.613 Ghz, 2flop/Hz,  5.226 Gflop
\end{itemize}
...
\begin{frame}{Free Performance}
 ...
 \end{block}
 \note{
  Most major perfromance gains come from major changes in structure. To take advantage of these changes users would need to modify and recompile their code. A way a user can avoid this is by using a BLAS that was already built to take advantage of the new hardware. \\

Major Performance Changes:\\
\begin{itemize}
 \item Barcelona 128bit SIMD double peak spead/hz
 ...
 \item \$8,000 Telsla reacheas 4 TFlop/s using GPU's \\
...
\begin{frame}{OpenMP and BLAS}
 \begin{block}{OpenMP}
  \begin{itemize}
   \item Type of Paralell Programming
   \item Shared Memory Paralell
...
To compile code with OpenMP on make sure you are using the MP version of a library.  Enable OpenMP in your comipler and link again the MP blas: \\
...
\begin{frame}{Fastest Fourier Transform in the West}
 ...
 \end{block}
 \note{
 Free library works very well. Fftw is portable across most HPC systems. The newest version even suports the CellBE. Not as flexable as BLAS and Lapack (not CPU vendor provided) but Fftw is the most portable high performance library available at no cost. \\
