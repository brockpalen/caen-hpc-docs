...
\begin{frame}{Single Program Multiple Data}
 \begin{block}{SPMD}
  ...
  \end{itemize}
 \end{block}
 \begin{block}{Example dgesvspmd.m}
  Solves Ax=b 
 \end{block}
\end{frame}

\begin{frame}[fragile]
\frametitle{dgesvspmd.m}
\begin{semiverbatim}
matlabpool mpiexec-works 10
N=20000
PART=N/matlabpool('size')
tic;
spmd
  A=rand(PART,N);
  dim=1;
  D=codistributed(A,codistributor('1d', dim));
  x=rand(PART,1);
...
\end{block}
\note{
The CAC via CAEN has unlimited licences to the NAG toolbox on MATLAB. It is not uncommon to use MATLAB as a frontend to parallel codes built as mex files.
}%
\end{frame}
  \section{Survey}
