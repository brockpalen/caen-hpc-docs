...
\begin{frame}{Hardware: Flux Allocations}
 ...
 \end{columns}
\note{
Flux is a cluster managed by the CAC that are available to researchers in a fee for service. There are also accomidations for grants that disallow fee for service. The minimum allocation is 1 core for 1 month. The goal of flux is quick and flexable access for researchers to get and adjust resources that match their needs.  Flux also offers GPGPUs and a few other resrouces. Contact flux-support@umich.edu for help.
} %note
\end{frame}
...
%compile code
  \begin{frame}{Compile Code}
   \note[item] { The following applies to the default modules, or the recomended flux modules}
 ...
 \begin{block}{MPI Code}
  \begin{itemize}
   ...
   \item <2-> \texttt{mpicc -fast -o c\_ex01 c\_ex01.c}
    ...
    \note[item]{\texttt{make -jN} will compile in perallel on N cores}
  \end{itemize}
 \end{block}
\end{frame}
...
\uncover{mpirun mcnp5 i=input2 o=output2 r=restart2}
  \end{semiverbatim}  
...
djsiege    0       -      djsiege cac,preempt,djsiege
  GDEF=meche
  UID=1913288
...
\begin{frame}{Getting Flux Allocations}
 ...
 \end{block}
\note{
Flux allocations look like your group purchased nodes. You have dedicated access to the N cores you have reserved.  If you rent 100 cores and don't run a single job on them you are still charged for us setting those nodes asside. Core counts can be changed.  Thus if a deadline is two months out your group can add 1000 more cores to your allocation for just those two months the cpu limits are changed by the system automaticaly.  Other specialty resources are priced differently (GPGPUS, Haddop, Large Memory).
} %note
\end{frame}
